...
public interface FieldCache {
	...
	public static class StringIndex {
		...
		public int binarySearchLookup(String key) {
			// this special case is the reason that Arrays.binarySearch() isn't
			// useful.
			if (key == null)
				return 0;
	...
	/**
	 * Marker interface as super-interface to all parsers. It is used to specify
	 * a custom parser to {@link SortField#SortField(String,
	 * FieldCache.Parser)}.
	 */
	 ...
	 * {@link NumericUtils#intToPrefixCoded(int)}, e.g. when indexed via
	 ...
	 * e.g. when indexed via {@link NumericField}/{@link NumericTokenStream}.
	 */
	 ...
	 * {@link NumericUtils#longToPrefixCoded(long)}, e.g. when indexed via
	 * {@link NumericField}/{@link NumericTokenStream}.
	 */
	...
	public static abstract class CacheEntry {
		...
		public void estimateSize() {
			estimateSize(new RamUsageEstimator(false)); // doesn't check for
														// interned
		}
	 ...
	 * <p>
	 * NOTE: These CacheEntry objects maintain a strong reference to the Cached
	 * Values. Maintaining references to a CacheEntry the IndexReader associated
	 * with it has garbage collected will prevent the Value itself from being
	 * garbage collected when the Cache drops the WeakRefrence.
	 * </p>
	 * <p>
	 * <b>EXPERIMENTAL API:</b> This API is considered extremely advanced and
	 * experimental. It may be removed or altered w/o warning in future releases
	...
	/**
	 * <p>
	 * EXPERT: Instructs the FieldCache to forcibly expunge all entries from the
	 * underlying caches. This is intended only to be used for test methods as a
	 * way to ensure a known base state of the Cache (with out needing to rely
	 * on GC to free WeakReferences). It should not be relied on for "Cache
	 * maintenance" in general application code.
	 * </p>
	 * <p>
	 * <b>EXPERIMENTAL API:</b> This API is considered extremely advanced and
	 * experimental. It may be removed or altered w/o warning in future releases
	...
	/**
	 * Expert: drops all cache entries associated with this reader. NOTE: this
	 * reader must precisely match the reader that the cache entry is keyed on.
	 * If you pass a top-level reader, it usually will have no effect as Lucene
	 * now caches at the segment reader level.
	 */
	public abstract void purge(IndexReader r);

	/**
	 * If non-null, FieldCacheImpl will warn whenever entries are created that
	 * are not sane according to
